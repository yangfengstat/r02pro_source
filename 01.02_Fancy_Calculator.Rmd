...
Before moving on to the natural logarithm, recall that R does not store the constant $e$ directly. Instead, you obtain it with `exp(1)`. The `exp()` function always returns $e$ raised to the power you provide, so `exp(3)` is $e^3$. The companion function `log()` computes logarithms, and when you omit the `base` argument it defaults to the natural logarithm (base $e$). The examples below show these ideas side by side.

```{r results = TRUE}
exp(1)
exp(3)
log(exp(3))              # natural logarithm: log(x, base = exp(1))
